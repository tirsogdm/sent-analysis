...
- Filtering with (5, 500) frequency range
"""
description = "Lemmas as features, filtering with (5, 500) frequency range"

def filtered(token, dataset):
    """
    Method called on each token to check if it should be filtered.
    """
    return not dataset.in_freq_range(token.lemma_, min_df=5, max_df=500)
